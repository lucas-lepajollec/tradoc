...
class SemanticChunker:
    def __init__(self, target_chunk_tokens: int = 1400, max_chunk_tokens: int = 1800):
    ...
    def create_chunks(self, nodes: List[str]) -> List[SegmentChunk]:
        ...
        for idx, node in enumerate(nodes):
            ...
            # If adding this node exceeds target window and we already have content
            if current_tokens + node_tokens > self.target_chunk_tokens and current_nodes:
                chunks.append(SegmentChunk(
                    index=chunk_counter,
                    text="\n\n".join(current_nodes),
